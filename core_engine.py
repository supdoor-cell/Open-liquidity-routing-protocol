...
# VERSION:     1.0.1 (Fixed Production Release)
        ...
        for root in range(start_node, end_node):
            with self._lock:
                ...
                for node_1 in list(self.hypergraph_registry[root].keys()):
                    if node_1 not in self.hypergraph_registry or not self.hypergraph_registry[node_1]: 
                        continue
                    for node_2 in list(self.hypergraph_registry[node_1].keys()):
                        if node_2 not in self.hypergraph_registry or not self.hypergraph_registry[node_2]: 
                            continue
...
if __name__ == "__main__":
    try:
        ...
        print(f"[ERROR] Core engine execution failed: {e}")
        raise
